...
/*   Updated: 2024/09/12 11:24:41 by tparratt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (str_arr[i])
	{
		...
		while (str_arr[i][j])
		{
					...
					|| int_arr[i] > 255))
			{
				free_2d(str_arr);
				free(int_arr);
				print_error("RGB contains alphabets or integer not in correct range",
					map);
			}
			j++;
		}
...
void	parse(char **argv, t_map *map)
{
	...
	if (invalid_file_extension(argv[1], ".cub"))
		print_error("Map should not be a directory and should have .cub file extension",
			NULL);
